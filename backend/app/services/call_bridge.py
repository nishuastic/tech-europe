...
from app.services.gradium_streaming import GradiumTTSStream, GradiumSTTStream, VOICE_IDS
...
class CallBridgeManager:
    ...
    async def initiate_call(self, user_message: str, target: str = "caf") -> CallSession:
        """
    ...
    async def _run_call_flow(self, session: CallSession):
        ...
        try:
            ...
            session.audio_url = f"{settings.backend_public_url}/api/v1/call/audio/{session.call_id}"
    ...
    async def handle_call_response(self, call_id: str, audio_data: bytes) -> str:
        ...
        try:
            ...
            session.english_response = await translate_to_english(session.french_response)
